...
type gcWork struct {
	...
	// Bytes marked (blackened) on this gcWork. This is aggregated into work.bytesMarked by dispose.
	// 此gcWork上标记（变黑）的字节。这将聚合到由dispose标记的work.Bytes中。
	bytesMarked uint64
...
/*
put不仅尝试将对象放入wbuf1，还会再wbuf1满的时候，尝试更换wbuf1 wbuf2的角色，都满的话，则想全局进行申请，
并将满的队列上交到全局队列
*/
...
//go:nowritebarrierrec
/*
work有wbuf1 wbuf2两个队列用于保存灰色对象，首先会往wbuf1队列里加入灰色对象，wbuf1满了后，交换wbuf1和wbuf2，
这时wbuf2便晋升为wbuf1，继续存放灰色对象，两个队列都满了，则向全局进行申请
*/
...
func (w *gcWork) balance() {
	...
	// 如果wbuf2不为空，则上交到全局，并获取一个空队列给wbuf2
