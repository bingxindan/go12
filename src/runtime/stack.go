...
//
//go:systemstack
// statckfree() 函数释放g的stacks信息,直到处理完所有g
...
func newstack() {
	...
	if thisg.m.morebuf.g.ptr() != thisg.m.curg {
		print("runtime: newstack c	alled from g=", hex(thisg.m.morebuf.g), "\n"+"\tm=", thisg.m, " m->curg=", thisg.m.curg, " m->g0=", thisg.m.g0, " m->gsignal=", thisg.m.gsignal, "\n")
...
// 调用 shrinkstack() 函数释放g的stacks信息,直到处理完所有g
func shrinkstack(gp *g) {
	...
	if gstatus&^_Gscan == _Gdead {
		if gp.stack.lo != 0 {
			...
			// if G is used again.
			// stackfree函数释放g的stacks信息,直到处理完所有g
			stackfree(gp.stack)
			gp.stack.lo = 0
			gp.stack.hi = 0
		}
